...
rm(list = ls())
setwd("/pasteur/homes/maylayan/MMMI_Rage/HKY_3demes/")
...
weightsRegions[2, 2:6] <- biasWeights # Region2 will be oversampled
...
H$aggregation[H$region %in% c("Gharb - Chrarda - Béni Hssen",
                              ...
                              "Chaouia - Ouardigha", 
                              "Doukkala - Abda",
                              "Marrakech - Tensift - Al Haouz")] <- "Region2"

H$aggregation[H$region %in% c("Souss - Massa - Draâ",
                              "Tadla - Azilal", 
                              "Guelmim - Es-Semara",
                              "Laâyoune - Boujdour - Sakia El Hamra")] <- "Region3"

human_pop = group_by(H, aggregation) %>% summarise(pop = sum(ind)) %>% .$pop

  # Human mobility
distance = as.matrix(read.table("../../Input_files/mobility_matrix_3demes.txt", 
	header = FALSE))
mobility = TRUE
...
epsilon <- 1
iota <- 0
connection_strength <- 0.00000002
...
if (args[1] == 1) seed = 1595347952
if (args[1] == 2) seed = 1605134975
if (args[1] == 3) seed = 1605126731
if (args[1] == 4) seed = 1605107615
if (args[1] == 5) seed = 1605134226
