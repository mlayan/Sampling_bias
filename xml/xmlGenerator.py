...
def xml_beast1_old(fileName, logEvery, chainLength, directory = None, 
	outputName = None, prefix = None, templateFile = "template_beast1.xml"):
	"""
	...
	Arguments are: 
		- fileName: name of the fasta file, the region file is deduced from the fasta file
	...
	# Add comment on the number of sequences
	...
	if outputName:
		...
		outputFileName = prefix + re.sub(".fasta", "", fileName)
	...
	for seq in range(0, nseq):
	    ...
	    # Append it to the taxa 
	    taxa.append(taxon)
	    
	    # Specify taxon attributes, add date 
	    date = etree.Element("date", value=format(seqDataFrame['dates'][seq]), direction="forwards", units="years")
	    taxon.append(date)
	    
	    # Do the same for the alignment
	    sequence = etree.Element("sequence")
	            
	    # Add taxon and sequence
	    taxon_seq = etree.Element("taxon", idref=format(seqDataFrame['traits'][seq]))
	    taxon_seq.tail = seqDataFrame['fasta'][seq]
	    sequence.text = None
	    sequence.append(taxon_seq)
	    
	    # Append the sequence
	    alignment.append(sequence)

	## Define the MCMC
	mcmc = template.getroot().find("mcmc")
	mcmc.set('chainLength', str(chainLength))
	mcmc.set('operatorAnalysis', outputFileName + ".ops.txt")

	## Change the steps at which data is collected
	for node in template.getroot().findall("mcmc/log"):
	    node.set('logEvery', str(logEvery))

	## Change file names
	for node in template.xpath('mcmc/log[@id="fileLog"]'):
		node.set('fileName', outputFileName + ".log.txt")

	for node in template.xpath('mcmc/logTree'):
		node.set('fileName', outputFileName + ".trees.txt")
		node.set('logEvery', str(logEvery))

	## Write the output file
	template.write(directory + outputFileName + ".xml", encoding="UTF-8", 
		standalone="yes", pretty_print=True, xml_declaration=True)

	# Write bash file
	bash_xml(fileName, directory, logEvery, beastVersion = 1)
	...
	if fixedTreeFile or fixedTreeDirectory:
		if not fixedTreeDirectory:
			...
			if not re.search(r'/$', fixedTreeDirectory):
				fixedTreeDirectory += "/"   
		tree = Phylo.read(fixedTreeDirectory + fixedTreeFile, 'newick')
		template = estimationOnFixedTree2(template, outputFileName, tree)
...
def estimationOnFixedTree(template, outputFileName, tree):
	...
	# Create the stateNode element which will replace the run/init element
	stateNode = etree.Element('stateNode', 
		spec='beast.util.TreeParser',
		id='Tree.t:' + outputFileName, 
		IsLabelledNewick='true', 
		adjustTipHeights='true', 
		taxa='@' + outputFileName,
		newick=Phylo.Newick.BaseTree.Tree.format(tree, 'newick').replace("\n", ""))

	# Remove the run/init element
	for node in template.xpath('run/init[re:match(@id, "^RandomTree")]',namespaces={'re':regexpNS}):
		node.getparent().replace(node, stateNode)
...
def estimationOnFixedTree2(template, outputFileName, tree):
	"""
