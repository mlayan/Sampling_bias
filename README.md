...
Simulations can be launched using the `R` scripts in the `generate` subfolders of `3demes` and `7demes`. These scripts depend on the following R libraries: `dplyr`, `Rcpp`, `seqinr`, `doParallel`, `doRNG`, and `foreach`. Script execution can be launched as follows: `./1.HKY_3demes.R simulation_batch n_cores` with `simulation_batch` an integer between 1 and 5 indicating the simulation batch, and `n_cores` the number of cores used to parallelize simulation computation. Simulated transmission chains, fasta files and traits files (location of the sampled sequences) are generated and stored in the following subfolders: `3demes/simulation1/files` or `7demes/simulation1/files`.

### XML generation
XML files are generated using an in-house python module based on the `lxml` python module and tailored BEAST XML template files. The python module and templates files are located in `python` and one can launch the python script as follows `./Generate_XML.py` to generate XML files from simulated data and template files. XML files are stored in `3demes/simulation1/"BEAST_model"` or `7demes/simulation1/"BEAST_model"`. `"BEAST_model"` subfolders corresponds to the BEAST models tested in the original analysis:
- `dta` (CTMC model)
- `mascot` 
- `basta`
...
Scripts to analyze the log and tree files generated by BEAST are located in `analyses`. They are parallelized on 10 CPUs and rely on multiple in-house python modules and the following Python libraries: `pandas`, `numpy`, `dendropy`, `Biopython`, `sklearn`, `scipy`, `io`, `concurrent`, and `itertools`.

One can launch the different analyses as follows:
- `./1.analyze_simulations.py n_demes`
- `./2.summarize_dta_basta_mascot.py n_demes`
- `./3.summarize_mascot_glm.py n_demes n_seq start_sim end_sim`
- `./4.get_lineage_migrations_introduction_dates.py n_demes beast_model n_seq`
- `./5.compare_topologies.py n_demes *models`
with `n_demes` equal to `3demes` or `7demes`; `n_seq` one of the sample sizes among `150` and `500`; `start_sim` and `end_sim` are integers indicating the range of the simulated epidemics that are to be analyzed; `beast_model` one of the beast models among `basta`, `dta`, `mascot`, and `glm`; `*models` any combination of beast models among `basta`, `dta`, `mascot`, and `glm`.
