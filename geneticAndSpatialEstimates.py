...
def logFileWrangler(fileName, directory, nMatrix, runType, 
	...
	# Number of regions 
	regions = [re.sub('rates_[a-zA-Z]*_', '', x) for x in log.columns.tolist() \
			if 'rates_' in x]
	...
	if len(regions) == (nRegions * (nRegions-1)) / 2:
		...
		raise ValueError("The number of migration rates {0} doesn't \
			correspond to the number of regions {1}".format(len(regions), nRegions))
...
def forwardsRates(log):
	...
	for col in log.columns:
		if 'rates_' in col:
			j = re.match(r'rates_([A-Za-z]*)_.*', col).group(1)
			i = re.match(r'.*_([A-Za-z]*)$', col).group(1)
